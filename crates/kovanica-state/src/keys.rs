...
impl Address {
    ...
    /// Version 0x05: Vault (time-lock vault / escrow) template hash (RFC-005).
    ...
    /// Construct a Version 0x05 (Vault) address by computing the BLAKE3 digest of a vault template.
...
mod tests {
    ...
    fn unsupported_version_rejected() {
        let mut bytes = [0u8; 33];
        bytes[0] = 0x06; // unsupported version (above VERSION_MAX)
    ...
    fn vault_address_properties() {
        ...
        assert_eq!(Address::parse(&shown).unwrap(), addr);
        assert_eq!(Address::parse(&addr.to_hex()).unwrap(), addr);
        // from_vault_script computes BLAKE3 of the template bytes
        let template = [0xCDu8; 72];
