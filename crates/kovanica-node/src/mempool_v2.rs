...
mod tests {
    ...
    fn utxo_with(op: OutPoint, value: u64) -> UtxoSet {
        ...
        utxo.insert(op, TxOutput::native(value, addr(1)), 0, false);
        utxo
    }
    ...
    fn fee_rate_ordering() {
        ...
        let mut utxo = UtxoSet::new();
        utxo.insert(op1, TxOutput::native(100_000, addr(1)), 0, false);
        utxo.insert(op2, TxOutput::native(100_000, addr(1)), 0, false);
        utxo.insert(op3, TxOutput::native(100_000, addr(1)), 0, false);
    ...
    fn orphan_promotion_on_new_block() {
        ...
        let mut utxo = UtxoSet::new();
        utxo.insert(op, TxOutput::native(1000, kp.address()), 0, false);
    ...
    fn capacity_limit_evicts_lowest_fee() {
        ...
        for i in 0..3 {
            ...
            utxo.insert(op, TxOutput::native(100, addr(1)), 0, false);
            let tx = Transaction::signed(
                &[(op, &kp)],
    ...
    fn remove_all_clears_both_pools() {
        ...
        let mut utxo = UtxoSet::new();
        utxo.insert(op1, TxOutput::native(1000, kp.address()), 0, false);
        pool.add(tx1.clone(), &utxo).unwrap();
    ...
    fn replace_by_fee_succeeds_with_sufficient_bump() {
        ...
        let op = OutPoint::new(TxId::from_bytes([1u8; 32]), 0);
        let mut utxo = UtxoSet::new();
        utxo.insert(op, TxOutput::native(10_000, addr(1)), 0, false);
    ...
    fn replace_by_fee_requires_conflict() {
        ...
        let op2 = OutPoint::new(TxId::from_bytes([2u8; 32]), 0);
        let mut utxo = UtxoSet::new();
        utxo.insert(op1, TxOutput::native(10_000, addr(1)), 0, false);
        utxo.insert(op2, TxOutput::native(10_000, addr(1)), 0, false);
    ...
    fn fee_estimate_at_capacity() {
        ...
        for i in 0..3u8 {
            let op = OutPoint::new(TxId::from_bytes([i; 32]), 0);
            utxo.insert(op, TxOutput::native(100_000, addr(1)), 0, false);
            // fee = 1000 + i*1000
