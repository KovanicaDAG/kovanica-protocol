...
TCP **:9000** is the only P2P path (libp2p/30333 removed). Clones work out of the box — the default `KOVANICA_PEERS` includes `seed.kovanica.online:9000` and `seed3.kovanica.online:9000`. Do **not** use the Cloudflare-proxied `explorer…` hostname; raw TCP :9000 does not pass it. Cloneable tree: [kovanica-node](https://github.com/KovanicaDAG/kovanica-node).
